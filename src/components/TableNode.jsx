...
export default function TableNode({ id, data, selected }) {
  const { setNodeCollapsed, setNodeColor } = useStore();
  const { table, collapsed, color } = data;
    ...
    >
      ...
      {!collapsed && (
        <div className="py-1.5">
          {/* PK fields first */}
          {pkFields.map(field => (
            <FieldRow key={field.name} field={field} tableId={id} accentColor={accentColor} isPK />
          ))}
          {pkFields.length > 0 && regularFields.length > 0 && (
            <div className="mx-3 my-1 border-t border-border/50" />
          )}
          {regularFields.map(field => (
            <FieldRow key={field.name} field={field} tableId={id} accentColor={accentColor} />
          ))}
        </div>
      )}
    </div>
...
function FieldRow({ field, tableId, accentColor, isPK }) {
  return (
    <div
      className="relative flex items-center px-3 py-1 hover:bg-surface-3 group transition-colors"
      style={{ minHeight: 32 }}
    >
      {/* Left handle */}
      <Handle
      ...
      <div className="w-4 shrink-0 mr-1.5 flex items-center justify-center">
        {isPK && (
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none" title="Primary Key">
        ...
        {field.ref && !isPK && (
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none" title="Foreign Key">
